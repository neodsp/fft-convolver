...
//! ```
//!
//! [`set_response()`](FFTConvolver::set_response) swaps the impulse response and
//! [`reset()`](FFTConvolver::reset) clears the tail after a seek. Both are
//! real-time safe.
//!
//! ## Which convolver?
//!
...
//! distributed.
//!
//! **[`FFTConvolver`]** uses a uniform block size and does everything on the
...
//!
//! **[`TwoStageFFTConvolver`]** adds a large tail block on top of a small head
...
//!
//! **[`ThreadedFFTConvolver`]** splits the response the same way but hands the
...
//! most for multi-channel signals: with [`TwoStageFFTConvolver`], every channel
...
//! [`TwoStageFFTConvolver`], because waiting for the worker costs more than it
...
//!
//! Average cost of one [`process()`](FFTConvolver::process) call, 512-sample
//! buffer:
//!
...
//!
//! [`TwoStageFFTConvolver`]'s spike is the algorithm and reproduces run after
...
//! ## Real-Time Safety
//!
//! [`process()`](FFTConvolver::process),
//! [`set_response()`](FFTConvolver::set_response) and
//! [`reset()`](FFTConvolver::reset) allocate nothing, take no locks and make no
//! syscalls. [`init()`](FFTConvolver::init) and
//! [`init_default()`](TwoStageFFTConvolver::init_default) allocate, so call them
//! before the audio thread starts.
//!
//! [`ThreadedFFTConvolver`] differs in three ways: it has no `set_response()`,
//! its [`process()`](ThreadedFFTConvolver::process) performs one thread wake per
